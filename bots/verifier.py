...
discord_token=json.dumps(json.load(open("../keys.json", "r"))['discord_token'])[1:-1]
...
s = requests.Session()
urlStream = 'https://talao.co/discord-bot/verifier_stream'
...
class MyClient(discord.Client):
    ...
    async def on_raw_reaction_add(self,payload):
        ...
        if len(select)==0:
            ...
            if len(select)==0:
                return
        urlToQuery='https://talao.co/discord-bot/init/'+select[0][0]
        card=select[0][0]
